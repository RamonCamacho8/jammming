...
function SearchBar(props: {onSearch: (searchString: string) => void}) {
  ...
  return (
    <form className="col-span-2 w-full flex flex-row rounded-md dark:bg-slate-700 p-4 bg-slate-100 justify-center items-center " onSubmit={onSubmit}>
      <input className=" w-full m-2 p-2 dark:bg-slate-800 rounded-md  text-slate-10 caret-indigo-600 placeholder:text-slate-40" value={searchString} required onChange={(e) => setSearchString(e.target.value)} placeholder="Search for a song." type="text" />
